    ...
    <div onClick={onClose} className='fixed right-0 top-0 px-2 py-4 overflow-scroll scrollbar-hide  z-50 left-0 bottom-0 justify-center items-center flex dropdown'>
      ...
      <div className='scrollbar-hide overflow-y-scroll max-h-[95vh]  my-auto  bg-white dark:bg-[#2b2c37] text-black dark:text-white font-bold shadow-md shadow-[#364e7e1a] max-w-md mx-auto  w-full px-8  py-8 rounded-xl'>
        ...
        <div className='relative flex   justify-between w-full items-center'>
          ...
          <img onClick={() => {setIsElipsisMenuOpen((prevState) => !prevState)}} src={elipsis} className='cursor-pointer h-6' />
        ...
        <div className="mt-8 flex flex-col space-y-3">
          ...
          >
            {columns.map((col, index) => (
              <option className="status-options dark:text-black" key={index}>
                {col.name}
              </option>
            ))}
          </select>
